...
如果您之前没有使用过微信客服机器人，那么请在 `开发配置` 中点击企业内部接入右侧的 `开始使用` 按钮，您应该会看到回调配置的页面。

![](8287fd9fec5823847e6b590dc3f0f545.png)
...
并且点击下方的两个随机获取，得到 `Token` 和 `EncodingAESKey`，复制到 AstrBot 配置的 `token` 和 `encoding_aes_key` 处，然后点击保存配置，等待适配器加载完成。

回到微信客服 `回调配置`，点击 `完成`。如果一切无误，将会显示 `已完成`（否则会显示类似 `openapi 回调不通过` 类似的文本）。

之后，在 `开发配置` 中得到企业 ID、Secret，找到复制到刚刚创建的企业微信适配器，点击编辑，然后修改配置中的 `corpid` 和 `secret`。然后再次保存配置，等待适配器加载完成。
