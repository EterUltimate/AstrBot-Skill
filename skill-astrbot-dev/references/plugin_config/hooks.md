...
- `@filter.on_llm_request()`
  ...
  - ⚠️ 注意：修改 `system_prompt` 后，如果缓存命中（上下文 hash 未变），系统可能仍用旧的 system_prompt。如需强制刷新，需同时修改 contexts。

- `@filter.on_llm_response()`
  - 触发：LLM 请求完成后（可读取/修饰返回结果）
  - 建议签名：`async def on_llm_response(self, event: AstrMessageEvent, response: LLMResponse) -> None`

### 3) Agent 运行阶段（>= v4.23.1）

- `@filter.on_agent_begin()`
  - 触发：Agent 工具循环开始前
  - 建议签名：`async def on_agent_begin(self, event: AstrMessageEvent) -> None`

- `@filter.on_agent_done()`
  - 触发：Agent 工具循环完成后
  - 建议签名：`async def on_agent_done(self, event: AstrMessageEvent) -> None`

### 4) 工具调用前后（Function Calling / Tools Use）
...
## extra_user_content_parts（>= v4.24.0）

在 `on_llm_request` 钩子中，可以通过 `request.extra_user_content_parts` 向 LLM 请求追加额外的用户消息段（如图片、文件等），而不修改用户原始消息。

```python
@filter.on_llm_request()
async def inject_context(self, event: AstrMessageEvent, request: ProviderRequest):
    from astrbot.api.message_components import Image
    part = Image.fromURL("https://example.com/diagram.png")
    part.mark_as_temp()  # 标记为临时，不持久化到对话历史
    request.extra_user_content_parts.append(part)
```

- `mark_as_temp()`: 标记该消息段为临时内容，不会被写入对话历史存储。适用于每次请求都需要动态注入但不需要持久化的场景。
- 不调用 `mark_as_temp()` 的 part 会随对话历史一起持久化。

## 优先级系统

钩子支持 `priority` 参数控制执行顺序：

```python
@filter.on_llm_request(priority=10)
async def high_priority_hook(self, event, request):
    ...
```
