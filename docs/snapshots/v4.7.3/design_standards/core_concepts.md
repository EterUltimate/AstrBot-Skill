...
- `event.unified_msg_origin`: 获取统一消息源 (UMO)。
- `event.send(result)`: 发送回复。
- `event.plain_result(text)`: 快速构建纯文本返回。
- `event.image_result(path)`: 快速构建图片返回。
- `event.chain_result(chain)`: 通过消息链构建返回。
- `event.make_result()`: 创建空结果容器。
